...
typedef INT         (WINAPI * LPD3DPERF_BEGevent)(D3DCOLOR, LPCWSTR);
...
static LPD3DPERF_BEGevent                 s_DynamicD3DPERF_Begevent = NULL;
...
static bool DXUT_EnsureD3D9APIs( void )
{
    ...
    if( s_hModD3D9 != NULL )
    {
        ...
        s_DynamicD3DPERF_Begevent = (LPD3DPERF_BEGevent)GetProcAddress( s_hModD3D9, "D3DPERF_Begevent" );
...
int WINAPI DXUT_Dynamic_D3DPERF_Begevent( D3DCOLOR col, LPCWSTR wszName )
{
    if( DXUT_EnsureD3D9APIs() && s_DynamicD3DPERF_Begevent != NULL )
        return s_DynamicD3DPERF_Begevent( col, wszName );
    else
        return -1;
}
                                               ...
                                               ID3D11DeviceContext** ppImmediateContext )
{
    if( DXUT_EnsureD3D11APIs() && s_DynamicD3D11CreateDevice != NULL )
	{
        ...
        //                                   SDKVersion, ppDevice, pFeatureLevel, ppImmediateContext );
			return D3D11CreateDevice(pAdapter, D3D_DRIVER_TYPE_UNKNOWN, NULL, 0, 0, 0, D3D11_SDK_VERSION,
			ppDevice, &level, ppImmediateContext);
	}
    else
        return DXUTERR_NODIRECT3D11;
}
